...
void output_log_layers(struct liftoff_output *output) {
	...
	liftoff_list_for_each(layer, &output->layers, link) {
		if (layer->force_composition) {
			liftoff_log(LIFTOFF_DEBUG, "  Layer %p (forced composition):", (void *)layer);
			continue;
		} else {
		...
		for (i = 0; i < layer->props_len; i++) {
			...
			if (strcmp(name, "CRTC_X") == 0 || strcmp(name, "CRTC_Y") == 0) {
				liftoff_log(LIFTOFF_DEBUG, "    %s = %"PRIi32, name, (int32_t)value);
				continue;
			}
			...
			liftoff_log(LIFTOFF_DEBUG, "    %s = %"PRIu64, name, value);
		}
	}
}
